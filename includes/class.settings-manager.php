...
class AdvSet_SettingsManager {
     ...
     */
    public function save_settings($settings, $return_change_status = false) {
        ...
        foreach ($features as $feature_id => $feature) {
            if (isset($settings[$feature_id])) {
                ...
                if (isset($ui_config['fields']) && is_array($ui_config['fields'])) {
                    ...
                    foreach ($ui_config['fields'] as $field_id => $field_config) {
                        if (isset($value[$field_id]) && isset($field_config['type'])) {
                            if (!$this->validate_field_type($field_config['type'], $value[$field_id], $field_config)) {
                                $errors[] = sprintf(
                ...
                // Clean up the value
                $cleaned_value = $this->cleanup_feature_value($value, $feature);
        ...
        // Generate cache file
        advset_cache_manager()->generate_cache_file();
     ...
     */
    private function cleanup_feature_value($value, $feature) {
        ...
        if (is_array($value) && isset($ui_config['fields'])) {
            ...
            foreach ($value as $field_id => $field_value) {
                ...
                if (!$this->is_empty_or_default_value($field_value, $ui_config['fields'][$field_id])) {
            ...
            $cleaned_value = $this->is_empty_or_default_value($value, $ui_config) ? null : $value;
        }
     ...
     */
    private function is_empty_or_default_value($value, $field_config) {
